              ...
              <div className="bg-white rounded-3xl shadow-md overflow-hidden border border-slate-100 animate-fade-in">
                ...
                <div className="p-6 md:p-8">
                  <span className={`inline-block text-xs px-3 py-1 rounded-full font-bold mb-3 uppercase ${
                    currentVideo.category === 'Theory' ? 'bg-purple-100 text-purple-700' : 
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                {videos.map(video => (
                  <div 
                    ...
                    className={`flex gap-4 p-3 rounded-2xl cursor-pointer transition border ${currentVideo?._id === video._id ? 'bg-purple-50 border-purple-200 shadow-sm' : 'border-transparent hover:bg-gray-50 hover:border-gray-200'}`}
                  >
                    <div className="w-32 h-20 bg-gray-200 rounded-xl overflow-hidden flex-shrink-0 relative shadow-sm">
                      ...
                      {currentVideo?._id === video._id && (
                        <div className="absolute inset-0 bg-purple-600 bg-opacity-60 flex items-center justify-center backdrop-blur-[1px]">
                          <span className="text-white text-xl shadow-sm">▶</span>
                        </div>
                      )}
                    </div>
                    <div className="flex-1 py-1 flex flex-col justify-center">
                      <h4 className={`font-bold text-sm line-clamp-2 leading-tight ${currentVideo?._id === video._id ? 'text-purple-900' : 'text-gray-800'}`}>
                        {video.title}
                      </h4>
